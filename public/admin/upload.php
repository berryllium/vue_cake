    ...
    if ($photo['tmp_name']) {
        ...
        if (in_array($photo['type'], $mas)) {
            if (move_uploaded_file($photo['tmp_name'], PATH_ROOT . $path_big . $photo_name)) {
                ...
                if ($id) {
                    $query = "UPDATE `products` SET `title` = '$title', `category` = '$category', `description` = '$desc', `price` = '$price', `units` = '$units', `path_big` = '$big', `path_small` = '$small' WHERE `products`.`id` = '$id';";
                } else $query = "INSERT INTO `products` (`id`, `title`, `description`, `category`, `price`, `units`, `path_big`, `path_small`) VALUES (NULL, '$title', '$desc', '$category', '$price', '$units', '$big', '$small');";
            }
        } else echo 'Можно загрузить только изображения в формате .jpg, .png или .gif';
    } else $query = "UPDATE `products` SET `title` = '$title', `category` = '$category', `description` = '$desc', `price` = '$price', `units` = '$units' WHERE `products`.`id` = '$id';";
